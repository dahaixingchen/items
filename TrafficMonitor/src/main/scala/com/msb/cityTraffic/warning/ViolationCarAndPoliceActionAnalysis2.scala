package com.bjsxt.traffic.warning
...
object ViolationCarAndPoliceActionAnalysis2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //要利用keyedStream才能进行keyed state进行状态编程
    var mainStream=stream1.keyBy(_.car).connect(stream2.keyBy(_.car))
      .process(new KeyedCoProcessFunction[String,ViolationInfo,PoliceAction,String] {
        ...
        lazy val vState: ValueState[ViolationInfo] = getRuntimeContext.getState(
          new ValueStateDescriptor[ViolationInfo]("v",classOf[ViolationInfo]))

        lazy val pState: ValueState[PoliceAction] = getRuntimeContext.getState(
          new ValueStateDescriptor[PoliceAction]("p",classOf[PoliceAction]))

        override def processElement1(value: ViolationInfo, ctx: KeyedCoProcessFunction[String, ViolationInfo, PoliceAction, String]#Context
                                     , out: Collector[String]) = {
          val policeAction:PoliceAction = pState.value()
          if(policeAction==null){//可能出警的数据还没有读到，或者该违法处理还没有交警出警
            ctx.timerService().registerEventTimeTimer(value.createTime+5000) //5秒（替代5min）后触发提示
            vState.update(value)
          }else{ //已经有一条与之对应的出警记录,可以关联
            out.collect(s"该违法车辆${value.car}，违法时间${value.createTime},已经有交警出警了" +
              s"，警号为:${policeAction.policeId},出警的状态是：${policeAction.actionStatus},出警的时间:${policeAction.actionTime}")
            //如果不clear的话，那有些违法记录可能超过了5s中没处理，后来处理了，也会进入到这里来，这样就不太适合了
            vState.clear()
            pState.clear()
          }
        }
        ...
        override def processElement2(value: PoliceAction, ctx: KeyedCoProcessFunction[String, ViolationInfo, PoliceAction, String]#Context, out: Collector[String]) = {
          ...
          if(info==null){ //出警记录没有找到对应的违法车辆信息
            ...
            out.collect(s"该违法车辆${info.car}，违法时间${info.createTime},已经有交警出警了" +
              s"，警号为:${value.policeId},出警的状态是：${value.actionStatus},出警的时间:${value.actionTime}")
            vState.clear()
            pState.clear()
          }
        }
        ...
        //触发器触发的函数
        override def onTimer(timestamp: Long, ctx: KeyedCoProcessFunction[String, ViolationInfo, PoliceAction, String]#OnTimerContext
                             , out: Collector[String]) = {
          val info: ViolationInfo = vState.value()
